...
{    
    ...
    //NSLog(@"Receipt str %@",[[NSString alloc] initWithBytes:transaction.transactionReceipt.bytes length:transaction.transactionReceipt.length encoding:NSUTF8StringEncoding]);
...
- (BOOL)doesTransactionInfoMatchReceipt:(NSString*) receiptString
{
    ...
    if ([[UIDevice currentDevice] respondsToSelector:NSSelectorFromString(@"identifierForVendor")]) // iOS 6?
    {
      ...
      //  NSString *localIdentifier           = [[UIDevice currentDevice] uniqueIdentifier];
        NSString *localIdentifier           = [[UIDevice currentDevice] uniqueGlobalDeviceIdentifier];
